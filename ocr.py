...
class Digits:
	...
	def get_label(input):
		...
		self.train, self.train_labels = __load_training()
		self.knn = cv2.KNearest()
		self.knn.train(self.train, self.train_labels)

		self.ret, self.result, self.neighbors, self.dist = knn.find_nearest(input, k=5)
		return self.result
	...
	def train_knn(self):
		...
		self.tester = self.digits[:,80:100].reshape(-1,400).astype(np.float32)
		...
		self.ret, self.result, self.neighbors, self.dist = knn.find_nearest(self.tester, k=5)
		...
		# save to file
		try:
			np.savez('digits_knn_train.npz', train=self.trainer, train_label=self.train_labels)
		except:
...
class Characters:
	def __init__(self):
		self.alph = np.loadtxt('letter-recognition.data', dtype='float32', delimiter=',',converters={0:lambda ch: ord(ch)-ord('A')})
	...
	def train_knn(self):
		...
		self.ret, self.result, self.neighbours, self.dist = self.knn.find_nearest(self.test_features, k=5)
		...
		# save training set
		try:
			np.savez('chars_knn_train.npz', train=self.train_features, train_label=self.train_labels)
		except:
			print ("Error writing training set: ", sys.exc_info()[0])
